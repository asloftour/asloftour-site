...
export async function POST(request: NextRequest) {
  ...
  const result = await handleProviderCallback({ provider: 'AKBANK' as any, body });
...
export async function GET(request: NextRequest) {
  ...
  const result = await handleProviderCallback({ provider: 'AKBANK' as any, body: query, query });
  return NextResponse.redirect(new URL(`/${result.locale}/${result.success ? 'success' : 'fail'}?reservation=${result.reservationId}`, request.url));
}
